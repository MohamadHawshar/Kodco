...
const Footer = () => {
  ...
  function sendEmail(){
    Email.send({
      ...
      Username : "username",
      Password : "password",
      To : 'mohamad_hawchar@hotmail.com',
      From : '',
    ...
    <section id='Contact' className={`bg-[#1C2D58] px-0 flex flex-col justify-center`}>
      ...
      <div className={`flex md:flex-row flex-col justify-between `}>
        <div className={`${styles.paddingY} ${styles.paddingX} flex-auto `}>
          ...
          <div className=''>
            <form action="https://formspree.io/f/xdojgqpk" method='POST' className={`flex flex-col `} enctype="multipart/form-data">
              <input type="text" id="fname" name="courriel" placeholder="Courriel" className={`mt-[25px] md:h-[40px] h-[35px] p-4`} />
              <input type="text" id="fname" name="nom" placeholder="Nom, Prénom" className={`mt-[15px] md:h-[40px] h-[35px] p-4`} />
              {/* <input type="text" id="lname" name="sujet" placeholder="Sujet" className={`mt-[25px] md:h-[50px] h-[35px] p-4`}/> */}
              <textarea id="lname" name="sujet" placeholder="Contenu" className={`mt-[15px] md:h-[140px] h-[140px] p-4 `} />
              <input type="file" className='mt-[15px] h-[50px] text-white' name='attachment' ></input>
              <input type="submit" value="Envoyer" className={`mt-[0px] bg-secondary md:h-[50px] h-[35px] uppercase text-white font-semibold cursor-pointer`} />
            </form>
          </div>
        </div>
